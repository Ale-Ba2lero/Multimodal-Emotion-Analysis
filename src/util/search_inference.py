...
class HashingMarginal(dist.Distribution):
    ...
    def expectation_mc(self, fun, monte_carlo_samples=1000):
        """
        Computes the expectation of the function fun w.r.t. the distribution.

        :param fun:
        :param monte_carlo_samples:
        :return:
        """
        samples = []
        ...
        counts = counts / monte_carlo_samples
        vals_fun = []
        for val in unique_vals:
            val_fun = fun(val)
            vals_fun.append(val_fun)
        vals_fun = np.array(vals_fun)
        prod = np.multiply(counts, vals_fun)
        return np.sum(prod)

    def expectation(self, loc, scale) -> torch.Tensor:
        """
        Calculates the expected value of the distribution here defined over an affine transformation:
          V(s) = scale * s + loc
        :param loc:
        :param scale:
        :return:
        """
        values = list(self._dist_and_values()[1].values())
        transformed_values = [
            loc + scale * val for val in values
        ]
        probs = [
            torch.exp(torch.tensor(self.log_prob(val))) for val in values
        ]
        expectation = torch.tensor(transformed_values) * torch.Tensor(probs)

        return torch.sum(expectation)

    def expectation_dbl(self, loc, scale, val_idx) -> torch.Tensor:
        """
        Calculates the expected value of the distribution here defined over an affine transformation:
          V(s) = scale * s + loc
        :param loc:
        ...
        values = list(self._dist_and_values()[1].values())
        transformed_values = [
            loc + scale * val[val_idx] for val in values
        ]
        probs = [
            torch.exp(torch.tensor(self.log_prob(val))) for val in values
        ]
