...
class SmallImgEncoder(nn.Module):
    def __init__(self, z_dim=64, hidden_dim=512, num_filters=128):
        super(SmallImgEncoder, self).__init__()
        self.nf = num_filters
        self.z_dim = z_dim
        self.features_output = self.nf * 8 * 8 * 8
...
class SmallImgDecoder(nn.Module):
    def __init__(self, z_dim=64, hidden_dim=512, num_filters=128):
        super(SmallImgDecoder, self).__init__()
        self.nf = num_filters
        self.z_dim = z_dim
        
        self.linear =  nn.Sequential(
            nn.Linear(z_dim, self.nf * 8 * 8 * 8),
            nn.ReLU())
